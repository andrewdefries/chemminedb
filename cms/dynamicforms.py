...
replace_res = \
    [re.compile('(<[^>]+) (%s=\\\\"[a-zA-Z0-9-_]+\.)#(\\\\")([^<]*>)'
     % s) for s in ('id', 'for', 'name')]
...
class BaseForm(forms.BaseForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs['prefix'] = kwargs.get('prefix', '') or getattr(self,
                'PREFIX', '')
        self.template = kwargs.pop('template', '') or getattr(self,
                'TEMPLATE', '')
    ...
    def add_prefix(self, field_name):
        ...
        return self.postfix and '%s.%s' % (prefixed, self.postfix) \
            or prefixed
