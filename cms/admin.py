...
class PageAdmin(admin.ModelAdmin):
    ...
    tinymce_js = ('js/getElementsBySelector.js',
                  'filebrowser/js/AddFileBrowser.js')

    class Media:

        css = {'screen': ('cms/css/admin.css', )}
        js = ('cms/js/mootools.js', 'cms/js/DateTimeShortcuts.js')
    ...
    def page_add_edit(self, request, id=None):
        ...
        if request.method == 'POST':
            if not add:
                ...
                pagecontent_data = \
                    [pagecontent_form.render_js('from_template')
                     for pagecontent_form in pagecontent_forms]
            if form.is_valid() and (add
                                    or pagecontent_forms.are_valid()):
                page = form.save()
                ...
                if not add:
                    ...
                    for pagecontent_form in pagecontent_forms:
                        if pagecontent_form.id:
                            try:
                                page_content = \
                                    PageContent.objects.get(pk=pagecontent_form.id)
                            except PageContent.DoesNotExist:
                                page_content = PageContent()  # Is this ok?
                        else:
                            page_content = PageContent()
                        set_values(page_content, PAGECONTENT_FIELDS,
                                   pagecontent_form.cleaned_data)
            ...
            if not add:
                pagecontent_data = \
                    [PageContentForm(initial=get_values(page_content,
                     PAGECONTENT_FIELDS),
                     id=page_content.id).render_js('from_template')
                     for page_content in page_contents]

        media = self.media
        if USE_TINYMCE:
            media.add_js(self.tinymce_js)
        media.add_js(('cms/js/dynamicforms.js', 'cms/js/page_add.js'))
    ...
    def navigation(self, request):
        ...
        def render(obj):
            ...
            return '<li id="navigation-%s">%s%s</li>' % (parent.id,
                    content, children and '<ul>%s</ul>'
        ...
        if request.method == 'POST':
            pages = dict([(page.id, page) for page in
                         Page.objects.all()])
            for form in NavigationForm.get_forms(request):
                ...
                try:
                    ...
                    page.in_navigation = \
                        form.cleaned_data['in_navigation']
            ...
            if len(hierarchy_data) == 1:
                ...
                error = \
                    _('Your changes were discarded, because there must not be more than one root element. Make sure that there is only one root element.'
                      )
        ...
        try:
            ...
            rendered_navigation = \
                _('<li>Please <a href="../page/add/">create a page</a> first.</li>'
                  )

        media = self.media
        media.add_js(('cms/js/nested.js', 'cms/js/navigation.js'))

        return render_to_response('cms/navigation.html', {
            'title': _('Edit pages'),
            'navigation': mark_safe('<ul id="navigation">%s</ul>'
                                    % rendered_navigation),
            'error': error,
            'media': mark_safe(media),
            'opts': opts,
    ...
    def page_preview(self, request, id):
        ...
        try:
            pagecontent_form = [form for form in
                                PageContentForm.get_forms(request)
                                if form.postfix == form_id][0]
        except IndexError:
        ...
        if pagecontent_form.is_valid():
            ...
            return HttpResponse(_('<h2>Your form is not valid.</h2>')
                                + smart_unicode(pagecontent_form.errors))

        return render_page(request, request.LANGUAGE_CODE, page,
                           preview=page_content.prepare())
