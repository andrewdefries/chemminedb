...
def render_pagecontent(page_content, context):
    ...
    if page_content.allow_template_tags:
        template = \
            Template('{%% load i18n cms_base cms_extras %s %%}{%% cms_pagination %d %%}%s{%% cms_end_pagination %%}'
                      % (' '.join(TEMPLATETAGS), context['page_number'
...
def render_page(
    ...
    if not Page.objects.root().published(request.user) \
        or not page.published(request.user):
        raise Http404
    ...
    if page.context:
        try:
            ...
            raise StandardError, \
                'The context function for this page does not exist. %s: %s' \
    ...
    for (n, position) in enumerate(POSITIONS):
        ...
        if n == 0:
            ...
            context.update({'page_content': page_content,
                           ...
                           or page_content.title})

        (title_dict[position], content_dict[position]) = \
            render_pagecontent(page_content, context)
    ...
    if preview:  # append _preview to template name
        if template_path.endswith('.html'):
            template_path_preview = \
                template_path[:template_path.rfind('.html')] \
                + '_preview.html'
        else:
...
def handle_page(request, language, url):
    ...
    pages = Page.objects.filter(override_url=True, overridden_url=url,
    ...
    if not parts and not DISPLAY_ROOT:
        try:
            ...
            raise RootPageDoesNotExist, \
                unicode(_('Please create at least one subpage or enable DISPLAY_ROOT.'
                        ))
    ...
    for part in parts:
        pages = parent.page_set.filter(Q(slug=part)
...
def handler(request):
    ...
    try:
        ...
        raise StandardError, \
            'Please add django.middleware.locale.LocaleMiddleware to your MIDDLEWARE_CLASSES.'
    ...
    if not language in languages:
        raise Exception, \
            _("Please define LANGUAGES in your project's settings.")
...
def search(
    ...
    if request.GET.get('query', False):
        ...
        if search_form.is_valid():
            ...
            search_results = Page.objects.search(request.user, query,
                    language)
            page_ids = [res['id'] for res in search_results.values('id'
                        )]
            search_results_ml = Page.objects.search(request.user,
                    query).exclude(id__in=page_ids)
    ...
    from screendb.views import search as screen_search
    screens = screen_search(request)
