...
class EditInlineNode(template.Node):
    ...
    def render(self, context):
        ...
        add_on_name = u'%s.%s' % (relation.model.__module__,
                                  relation.model.__name__)
        if add_on_name in cms_settings.PAGE_ADDONS:
            context.push()
            original = context.get('original', None)
            bound_related_object = relation.bind(context['form'],
                    original, bound_related_object_class)
            context['bound_related_object'] = bound_related_object
            t = \
                loader.get_template(bound_related_object.template_name())
...
def edit_inline(parser, token):
    ...
    if len(bits) != 2:
        raise template.TemplateSyntaxError, '%s takes 1 argument' \
            % bits[0]
