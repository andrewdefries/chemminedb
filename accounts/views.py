...
def login(request, template_name='registration/login.html',
		redirect_field_name=REDIRECT_FIELD_NAME, force_get=False):
...
def cm_login(request, *args, **kargs):
	...
	if request.method == 'POST' and request.POST.get('newaccount'):
		...
		if isinstance(rg_resp, HttpResponseRedirect):
			...
			resp.update(rg_resp)
		login_resp = login(request, force_get=True, redirect_field_name='l', *args, **kargs)
	else:
		login_resp = login(request, redirect_field_name='l', *args, **kargs)
	...
	return render_to_response(
		kargs.get('template_name', 'registration/login.html'),
		resp,
...
def cm_useradd(request, *args, **kargs):
	...
	if form.is_valid():
		user = User.objects.create_user(
			username=md5(str(form.cleaned_data['email'])).hexdigest()[:30],
