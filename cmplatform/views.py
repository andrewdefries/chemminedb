from cmplatform import (models, forms, invoke_app, read_running_app_input,
	write_running_app_output, find_runing_app, update_status)
...
try:
	from moleculeformats import batch_sdf_to_smiles, batch_smiles_to_smiles
except:
	import sys
...
def startapp(request, app_id, input):
	try:
		...
		request.user.message_set.create(message=
				"Error:No such application: #%s" % app_id
		)
...
def do_add_user_object(request, data, name, type=None, source=None,
		source_class=None):
	# if type is not given, try parse as JSON. fall back to pure text
	if type is None:
		try: json_obj = models.JSONData(data, type="json")
		except: json_obj = models.JSONData(data, type="text")
	...
	if request.user.email: json_obj.serialize(request.user)
...
def startpage(request):
	...
	if request.method == 'GET':
		...
		apps = models.Application.objects.filter(app_type__in=[
			models.Application.REGULAR]).filter(input_type='text/smiles')
		...
		if 'upload-format' not in request.POST:
			error = "Error: Invalid form submission. Did you have JavaScript" \
					" enabled?"
		...
		if not error:
			...
			try:
				...
				if format == 'workspace':
					...
					input, n_err = batch_smiles_to_smiles(request.POST['smiles'])
				elif format == 'sdf-input':
					input, n_err = batch_sdf_to_smiles(request.POST['sdftext'])
				elif format == 'sdf-upload':
					input, n_err = batch_sdf_to_smiles(request.FILES['sdf'].read())

				if not input:
					raise Exception
			except:
				error = "Error: We cannot process your upload. It might be empty"\
						 " or invalid."
		...
		if not ajax:
			...
			if error:
				...
				if n_err:
					response['info'] = 'Warning: %d compounds cannot be'\
						' processed' % n_err
...
def shortlist(request):
    if request.is_ajax():
        ...
        output_type = models.Application.objects.filter(app_type__in=[models.Application.REGULAR]).filter(name=currentApp)[0].output_type
        # get list of apps matching that format
        apps = models.Application.objects.filter(app_type__in=[
            models.Application.REGULAR]).filter(input_type=output_type)
        for app in apps:
...
def app(request, object_id, queryset, *args, **kargs):
		...
		if obj_names:
			...
			for obj_name in obj_names.split('|'):
				if obj_name not in request.session['user_objects']:
					...
					request.user.message_set.create(message=
							"Error:No object with name '%s' could be found" % obj_name
					)
					return HttpResponseRedirect(request.get_full_path())
				input.append(str(request.session['user_objects'][obj_name]))
...
def limit_to_user(f):
	...
	user=request.user
	"""
	def decorated(request, queryset, *args, **kargs):
...
def handle_error(f):
	...
	def decorated(*args, **kargs):
		try:
			return f(*args, **kargs)
		except (ApplicationNotFoundError, RunningApplicationNotFoundError,
			RunningApplicationStatusError,
			TokenMismatchError, RunningApplicationStatusError), e:
...
@limit_to_user
def running_app(request, object_id, queryset, *args, **kargs):
	...
	if request.method == 'GET':
		...
		if r_app.status == models.RunningApplication.FINISHED:
			...
			if 'user_objects' in request.session:
				for name in request.session['user_objects']:
					if request.session['user_objects'][name].source and \
						request.session['user_objects'][name].source == r_app:
						saved = name
			# compatible next-steps; show nothing if error
			if error: _ = []
			else:
				_ = models.Application.objects.filter(
						input_type=r_app.application.output_type)
				_ = list(_)
			viewers = [i for i in _ if i.app_type == models.Application.VIEWER]
			processors = [i for i in _ if i.app_type == models.Application.REGULAR]
		return render_to_response('runningapplication_detail.html',
			dict(object=r_app, saved=saved, error=error,
				running=r_app.status in [models.RunningApplication.RUNNING,
					models.RunningApplication.INVOKED],
				viewers=viewers, redirect=redirect,
		...
		if 'processor' in request.POST and request.POST['processor']:
			return startapp(request, request.POST['processor'], r_app.stdout)
		else:
			uo_name = request.POST.get('name')
			if '|' in uo_name:
				request.user.message_set.create(message=
						"Error:'|' is not allowed in object names"
				)
				return HttpResponseRedirect(request.get_full_path())
			if not uo_name:
				request.user.message_set.create(message=
				...
				return HttpResponseRedirect(request.get_full_path())
			do_add_user_object(request, r_app.stdout, uo_name, source=r_app,
					source_class=r_app.application,
					type=r_app.application.output_type)
			return HttpResponseRedirect(request.get_full_path())

@limit_to_user
def list_running_apps(request, queryset, *args, **kargs):
	...
	if request.method == "GET":
		...
		if ra_ids:
			...
			request.user.message_set.create(message=
				"%d %s deleted" % (
					len(ra_ids),
					len(ra_ids) > 1 and 'applications' or 'application'))
		else:
			request.user.message_set.create(message=
				"No application have been deleted")
		return HttpResponseRedirect(request.get_full_path())
...
def add_user_object(request, *args, **kargs):
	...
	# on POST, process the input
		...
		if form.is_valid():
			...
			# check name
			if not form.cleaned_data['override'] and \
				uo_name in request.session.get('user_objects', dict()):
			...
			if form.cleaned_data['pure_text']:
				type="text/plain"
			do_add_user_object(request, form.cleaned_data['content'], uo_name,
					type=type)
