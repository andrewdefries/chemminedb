...
def invoke_app(user, app_id, input):
	...
	try:
		...
		x = urlopen(app.invoking_url + _ +
			'chemmine_id=%s&chemmine_token=%s' % (r.id, r.token))
	except:
...
def find_runing_app(r_app_id, token, mask_died=True, silent=False):
	r_app = models.RunningApplication.objects.filter(pk=r_app_id)
	if r_app.count() != 1:
		if silent: return None
		else: raise RunningApplicationNotFoundError 
	r_app = r_app.get()
	if r_app.token != token:
		if silent: return None
		else: raise TokenMismatchError
	if r_app.status in models.RunningApplication.died_status and mask_died:
		logging.info(
			"Finished/Failed RunningApplication instance %s masked" %
			r_app.id)
		if silent: return None
		else: raise RunningApplicationNotFoundError 
	return r_app
...
def update_status(r_app, status, silent=True):
	if status == r_app.status:
		...
		logging.error("Finished/Failed RunningApplication instance"
			" %s cannot change status" % r_app.id)
		if not silent: raise RunningApplicationStatusError
	elif r_app.status > status:
		logging.error("RunningApplication instance"
			" %s tries to reverse status" % r_app.id)
		if not silent: raise RunningApplicationStatusError
	else:
		old_status = r_app.status
		r_app.status = status
		logging.info("RunningApplication instance %s status updated"
			" from %s to %s" % (r_app.id, old_status, status))
		r_app.save()
