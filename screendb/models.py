...
SCREEN_TYPE = (
	('g', 'visiable to registered users'),
	('o', 'visiable to the owner only'),
	('x', 'visiable to everybody'),
)
...
class Screen(models.Model):
	...
	def search(cls, available_screens, query):
		...
		queryset = Screen.objects.filter(id__in=ids).filter(
...
class ScreenForm(forms.ModelForm):
	...
	description = forms.CharField(widget=Textarea(attrs={'cols':'30', 'rows':'2'}))
	funding = forms.CharField(widget=TextInput(attrs={'size':'30'}))
	strategy = forms.CharField(widget=Textarea(attrs={'cols':'30', 'rows':'2'}))
	extra_annotation = forms.CharField(widget=TextInput(attrs={'size':'30'}),
		required=False)

	class Meta:
		model = Screen
		exclude = ['owner', 'create_time', 'last_update', 'inactive_compounds']

	def clean(self):
		cleaned_data = self.cleaned_data
		from screendb.templatetags.annotationformat \
			import parse, annotation_to_li, SyntaxError
		value = cleaned_data.get('strategy', '')
		if ',' in value or '{' in value or '}' in value:
			try: parse(value)
			except SyntaxError, inst: 
				self._errors['strategy'] = self.error_class([
						'Syntax error: ' + inst.description +
						":: pos = " + str(inst.offset) + 
						" near '" + value[inst.offset:inst.offset+20] + "'"])
		elif ':' in value:
			try: annotation_to_li(value, lambda x:x)
			except:
				self._errors['strategy'] = self.error_class([
						'Syntax error '])
		return cleaned_data
...
class Publication(ScreenFile):
	...
	mode = models.CharField(choices=PUBLICATION_MODE_CHOICES, max_length=256)
...
class PublicationForm(forms.ModelForm, CompoundInputMixIn):
	...
	pub_title = forms.CharField(widget=TextInput(attrs={'size':'30'}),
		required=False)
	publication_url = forms.CharField(widget=TextInput(attrs={'size':'30'}),
		required=False)
	...
	def clean(self):
		...
		if mode == 'pubmed':
			if not pubmed_id:
				self._errors['pubmed_id'] = self.error_class([
					'This field is required when mode is set to PubMed ID'])
			else:
				...
				try:
					...
					self.cleaned_data['pub_title'] = abstract['pub_title']
					self.cleaned_data['author'] = abstract['author']
					self.cleaned_data['url'] = abstract['url']
				except:
					self._errors['pubmed_id'] = self.error_class([
						'This does not seem to be a valid PubMed ID'])
				
		elif mode == 'publication':
			required = ['journal', 'volume', 'pages', 'pub_title', 'author']
			for field in required:
				if not cleaned_data.get(field):
					self._errors[field] = self.error_class([
						'This field is required when mode is set to Publication Details'])
		elif mode == 'web':
			if not cleaned_data.get('web_url'):
				self._errors['web_url'] = self.error_class([
					'This field is required when mode is set to Web Address'])
		else:
			self._errors['mode'] = self.error_class([
				'Please select one mode from the dropdown menu'])
...
class StandardCompoundAnnotation(ScreenFile):
	...
	# assay 3 (optional)
	a3_name = models.CharField(max_length=256, blank=True, verbose_name="Assay Name")
...
class StandardCompoundAnnotationForm(forms.ModelForm, CompoundInputMixIn):
	compound = forms.CharField(widget=TextInput(attrs={'size':'30'}),
		required=False)
	title = forms.CharField(widget=TextInput(attrs={'size':'30',
	...
	def clean(self):
		...
		if not cleaned_data.get('compound'):
			self._errors['compound'] = self.error_class([
				'You must specify the compound to be annotated'])
		...
		required = [
			['a2_name', 'a2_score', 'a2_concentration'],
				...
				for i in assay:
					if not cleaned_data.get(i):
						self._errors[i] = self.error_class([
							'This field is required if you enable this assay'])
			...
			try:
				...
				if not mime.startswith('text/'):
					self._errors['path'] = self.error_class([
						'The uploaded file does not seem to be a text file.'])
...
class ImageFileForm(forms.ModelForm, CompoundInputMixIn):
	compound = forms.CharField(widget=TextInput(attrs={'size':'30'}),
		required=False)
	title = forms.CharField(widget=TextInput(attrs={'size':'30'}))
	reference = forms.FileField(help_text="Display a reference image next to the above image (optional)", required=False)
	class Meta:
		exclude = ['screen', 'mime', 'original_name', 'ordering']
		model = ImageFile
		...
		if 'path' in cleaned_data:
			try:
				...
				if not mime.startswith('image/'):
					self._errors['path'] = self.error_class([
						'The uploaded file does not seem to be an image file.'])
				if cleaned_data['reference'] and \
				not cleaned_data['reference'].content_type.startswith('image/'):
					self._errors['reference'] = self.error_class([
						'The uploaded file does not seem to be an image file.'])
	...
	def clean(self):
		...
		if 'path' in cleaned_data:
			try:
				...
				if not mime.startswith('text/'):
					self._errors['path'] = self.error_class([
						'The uploaded file does not seem to be an annotation file.'])
...
def pubmed_abstract(pubmed_id):
	...
	pattern = compile(r'div\s+class="rprt\s+abstract"><p\s+class="citation"><a\s+.*?>(.*?)</a>(.*?)</p><h1\s+class="title">(.*?)</h1><p\s+class="auth_list">(.*?)</p>')
...
class DTSEntry(models.Model):
	...
	control = models.CharField(max_length=1, choices=(('-', 'regular compound'), ('N', 'negative control'), ('P', 'positive control')))
	class Info:
		all_fields = []
	...
	def __unicode__(self):
		...
		if self.compound is None:
			...
			x = self.compound.library.all()[0].header.name, self.compound.cid
			if self.control == '-':
...
class DTS249Entry(DTSEntry):
	...
	class Info:
		per_plate = 384
		all_fields = ['raw', 'score', 'comment', 'plate_z1', 'plate_z',
	...
	def as_tds(self):
		...
		if url:
			table = """<td>%s</td><td class="compoundid"><a href="%s">%s</a></td>""" % (
				x[0], url, x[1])

		return table + \
		"""<td class="hidden_value">%s</td>""" % self.plate + \
		"""<td>%s</td>""" % self.well + \
		"""<td>%s</td>""" % self.raw + \
		"""<td>%s</td>""" % self.score + \
		"""<td>%s</td>""" % self.comment + \
