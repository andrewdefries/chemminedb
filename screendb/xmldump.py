...
....and invoke the dump process"""
    ...
    fname = os.path.join(screen_xml_dir, 'screen-%s.tar.bz2'
                         % screen.id)
    ready = os.path.isfile(fname) \
        and datetime.fromtimestamp(os.stat(fname)[stat.ST_CTIME]) \
        > screen.last_update
    if not ready:
...
def dump(sid):
    ...
    try:
        ...
        from screendb.models import Screen, Compound, \
            StandardCompoundAnnotation, Publication
        try:
        ...
        for f in files:
            ...
            if ff.compound:
                ...
                if compound_oldest_file[c] is None \
                    or compound_oldest_file[c] > ff.id:
            ...
            def _copy(fid, fpath, suggested_name):
                if not fpath in copied:
                    ...
                    name = '%d-%s' % (fid, suggested_name)
                    shutil.copy(fpath, os.path.join('screen-%s' % sid,
                                'data', name))
            ...
            if hasattr(ff, 'reference') and ff.reference:
                ff.reference_name = _copy(ff.id, ff.reference.path,
                        'reference')
        ...
        of = file(os.path.join('screen-%s' % sid, 'screen.xml'), 'w')
        for i in results:
            of.write(i)
        of.close()
        ...
        status = \
            os.system('tar -cjf screen-%s.tar.bz2 screen-%s > /dev/null'
                       % (sid, sid))
        if status == 0:
