...
def perm(user, screen, action):
    ...
    if action.startswith('read'):
        if screen is None:
            ...
            return True
        return screen.type == 'x' or screen.owner == user \
            or not user.username.startswith('guest_') and screen.type \
            == 'g' or not isinstance(user, auth.models.AnonymousUser) \
            and screen.type == 'g'
...
def get_all_screens(request):
    ...
    if request.user.is_superuser:
        ...
        if not isinstance(request.user, auth.models.AnonymousUser):
            screens = \
                list(Screen.objects.filter(owner=request.user).order_by('id'
                     ))
        else:
            screens = []
...
def list_all_screens(request):
    if request.method == 'POST':
        ...
        if not perm(request.user, None, 'update'):
            request.user.message_set.create(message="You don't have the permission"
                    )
        ...
        if user_data.is_valid():
            ...
            return render_to_response('screen_list.html',
                    dict(upload_form=user_data),
...
def delete_screen(request, screen_id):
    if request.method == 'POST':
        ...
        if perm(request.user, screen, 'delete'):
            ...
            request.user.message_set.create(message='Screen #%s has been deleted'
                     % request.POST['screenid'])
            return HttpResponseRedirect(reverse('all-screens'))
        else:
            request.user.message_set.create(message="You don't have permission."
                    )
            return HttpResponseRedirect(reverse('screen-detail',
                    kwargs=dict(screen_id=screen_id)))
    else:
        raise Http404


all_form_types = (  # ....('Extra Annotation', ExtraAnnotationForm),
...
def screen_mod(request, screen):
    ...
    if request.method == 'GET':
        if request.GET['func'] == 'edit':
            ...
            return HttpResponse(dumps(dict(form=Template('{{form.as_table}}'
                                ).render(Context(dict(form=f))))),
                                mimetype='text/plain')
            ...
            try:
                ...
                cls = getattr(module.models,
                              screenfile.__class__.__name__ + 'Form')
    ...
    if request.method == 'POST' and 'func' in request.POST:
        ...
        if request.POST['func'] == 'edit-file':
            ...
            module = __import__('screendb')
            cls = getattr(module.models, screenfile.__class__.__name__
                          + 'Form')
        ...
        if request.POST['func'] == 'inactives':
            ...
            if 'inactives' in request.FILES:
                ...
                if request.FILES['inactives'].multiple_chunks():
                    return HttpResponse(dumps(dict(status='error',
...
def screen_download(request, screen):
    ...
    if request.GET['download'] in ['ids', 'inactives']:
        ...
        if request.GET['download'] == 'ids':
            compounds = \
                Compound.objects.extra(tables=['screendb_screenfile'],
                    where=['screen_id=%d' % screen.id,
                    'compound_id=compounddb_compound.id']).distinct()
        else:
...
....or '>1', etc. return the modified queryset"""
    ...
    if table_clause:
        tables.append('screendb_screenfile')
    return q.extra(tables=tables, where=constraint
                   + ['screendb_standardcompoundannotation.screenfile_ptr_id=screendb_screenfile.id'
...
def screen_detail(request, screen_id):
    ...
    if 'ajax' in request.GET and 'func' in request.GET or 'ajax' \
        in request.POST and 'func' in request.POST:
    ...
    screen_files = \
        s.screenfile_set.filter(compound=None).order_by('ordering', 'id'
            )

    # GlobalReferenceImageFile is not subclass of ScreenFile

    try:
        global_reference = s.globalreferenceimagefile_set.all()[0]
    except:
    ...
    compounds = \
        compounds.extra(select={'min_fid': '''select min(screendb_screenfile.id) from 
			screendb_screenfile where
			screendb_screenfile.compound_id=compounddb_compound.id and
			screen_id=%d'''
                        % screenid}, tables=['screendb_screenfile'],
    ...
    if cache.get(cache_key) is not None:
        ...
        stats['n_library'] = \
            LibraryHeader.objects.extra(tables=['screendb_screenfile',
                'compounddb_compound_library', 'compounddb_library'],
                where=['compounddb_compound_library.compound_id=screendb_screenfile.compound_id'
                ,
                'compounddb_compound_library.library_id=compounddb_library.id'
                ,
                'compounddb_library.header_id=compounddb_libraryheader.id'
                , 'screendb_screenfile.screen_id=%s'],
                params=[screenid]).distinct().count()
    ...
    if stats_key in stats:
        ...
        for i in ACTIVE_LEVEL_CHOICES:
            _.append(add_score_filter(compounds, screenid, '>=%s'
                     % i[0], score2, score3).distinct().count())
    ...
    if score1 or score2 or score3:
        ...
        compounds = add_score_filter(compounds, screenid, score1,
                score2, score3)
    else:
    ...
    compounds = [[c, [i for i in files if i.compound == c]] for c in
                 compounds]
    ...
    if 'ajax' in request.GET:
        ...
        context = dict(RENDERER_URL=settings.RENDERER_URL,
                       hideheader=True)
        context['compounds'] = compounds
        resp = dict(html=t.render(Context(context)), n=len(compounds))
        resp['compounds'] = [(c[0].id, c[0].libname + ' ' + c[0].cid)
                             for c in compounds]
    ...
    click_pager = request.GET.get('page', None) is not None \
        and http_referer.endswith(request.META['PATH_INFO'])
    ...
    return render_to_response('screen_detail.html', dict(
        ...
        screen_level_files=screen_files,
        page_obj=page,
        ...
        score_filter_on=score_filter_on,
    ...
    if request.method == 'GET':
        ...
        if f.is_valid():
            ...
            if ajax:
                ...
                from django.conf import settings
                context.update(dict(RENDERER_URL=settings.RENDERER_URL,
                               hideheader=True))
                ...
                resp = dict(file=t.render(Context(context)),
                            form=Template('{{form.as_table}}'
                            ).render(Context(dict(form=form_cls()))),
            ...
            if ajax:
                resp = dict(form=Template('{{form.as_table}}'
                            ).render(Context(dict(form=f))))
                return HttpResponse(dumps(resp), mimetype='text/plain')
            else:
                return render_to_response('add_file_standalone.html',
                        dict(form=f, type=form_type,
                        screen_id=screen_id),
                        context_instance=RequestContext(request))
...
def delete_file(request, file_id, typehint):
    ...
    except (ScreenFile.DoesNotExist,
            GlobalReferenceImageFile.DoesNotExist, ValueError):

        raise Http404
    screen = f.screen
    if perm(request.user, screen, 'delete'):
        f.delete()
        request.user.message_set.create(message='File has been deleted.'
                )
    else:
        request.user.message_set.create(message="You don't have permission."
                )
    ...
    return HttpResponseRedirect(reverse('screen-detail',
                                kwargs=dict(screen_id=screen.id)))
...
def dts(request, screen_id):
    ...
    if page:
        if c:
            ...
            return HttpResponseRedirect(request.path + '?page=%d'
                    % page)
    ...
    if request.GET.get('hl', False):
        try:
            ...
            pass
    return object_list(request, queryset, template_name='dts.html',
                       extra_context=dict(
        screen_id=screen_id,
        cls=cls,
        highlight=highlight,
...
def virtualplate(request, screen_id):
    ...
    all_plates = cls.objects.all().order_by('plate').values_list('plate'
            ).distinct()
    paginator = Paginator(all_plates, 1)
    ...
    all_data = dict([(field, dict()) for field in
                    cls.Info.virtual_plate_fields])
    plate_data = []
    ...
    for row in range(x):
        ...
        for column in range(y):
            ...
            for field in cls.Info.virtual_plate_fields:
                try:
                    all_data[field][well] = \
                        float(getattr(welldata[well], field))
                    anywell = well
                except KeyError:
                    pass
    ...
    return render_to_response('dts-vp.html', dict(
        plate_data=plate_data,
        columns=cls.Info.virtual_plate_fields,
        screen_id=screen_id,
        g_min_max=dumps(g_min_max),
