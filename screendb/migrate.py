...
def format_tree(s, level=0):
    ...
    for i in s:
        ...
        if i[2]:
            r.append(format_tree(i[2], level + 1))
    return '\n'.join(r)
...
from screendb.models import Screen, User, StandardCompoundAnnotation, \
    ImageFile, TextFile, GlobalReferenceImageFile, Publication, \
    OtherFile, AnnotationFile
...
for c in compounds.values():
    ...
    compound = Compound.objects.get(cid__iexact=c['name'][0][1],
                                    library=lib)
...
for fid in files:
    ...
    if mime.startswith('image'):
        if 'reference' in screen and f['number'] == screen['reference']:
            ...
            GlobalReferenceImageFile(screen=s, id=fid, mime=mime,
                    path='import/%d/%s' % (s.id, f['original name'
                    ])).save()
        else:
