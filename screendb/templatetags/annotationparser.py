	...
	while True:
		...
		if match is None:
			if error_pattern.match(desc_string, pos):
				raise SyntaxError(pos, 'cannot find the start of a name-value pair: no name pattern found')
			break
		pos = match.end()
		key = match.group(1).rstrip()
		# extract the value
		for pattern in [quoted_value_pattern, value_pattern2, value_pattern, None]:
			# last resort: try recurssion with empty value ( key:  { ...)
			if pattern is None:
				entry = (key, '', [])
				if parent is not None: parent.append(entry)
				...
				try:
					...
					raise SyntaxError(exception.offset + pos, exception.description)
				break
			# for normal key extraction
			match = pattern.match(desc_string, pos)
			if match is not None:
				entry = (key, string.join(match.groups(), separator).rstrip(), [])
				if parent is not None: parent.append(entry)
				else: entries.append(entry)
				pos = match.end()
