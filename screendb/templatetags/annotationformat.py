...
def annotation_to_li(s, esc):
	...
	try:
		while True:
			if level == prev_level:
				html += '<li><span class="key">%s</span><span class="value">%s</span></li>' % (esc(key), esc(value))
				line = ''
				while not line.strip():
					try: line = lines.pop(0)
					except: break
...
def tree_to_li(tree, esc):
	...
	for name, value, subtree in tree:
		...
		if subtree:
			html += '<li><span class="key">%s</span><span class="value">%s</span><ul>%s</ul></li>' % (name, value, tree_to_li(subtree, esc))
		else:
			html += '<li><span class="key">%s</span><span class="value">%s</span></li>' % (name, value)
	return html
