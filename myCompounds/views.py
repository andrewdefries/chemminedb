...
def uploadCompound(request, *args, **kargs):
    ...
    if request.method == 'GET':
        return render_to_response('submitCompound.html', dict(p=page,
                                  matches=matches),
        ...
        if 'deleteSelected' in request.POST:
            ...
            if cids:
                ...
                request.user.message_set.create(message='Selected Compounds Deleted!'
                        )
                ...
                request.user.message_set.create(message='No Compounds Selected for Deletion!'
                        )
        if 'sendTools' in request.POST:
            ...
            if cids:
                ...
                request.user.message_set.create(message='Selected Compounds sent to Tools!'
                        )
                ...
                request.user.message_set.create(message='No Compounds Selected!'
                        )
        if 'deleteAll' in request.POST:
            input_mode = 'view'
            deleteMyCompounds(request.user.username)
            matches = None
            request.user.message_set.create(message='All Compounds Deleted!'
                    )
        if 'smiles' in request.POST:
            ...
            if len(filteredCIDs) > 0:
                try:
                    ...
                    request.user.message_set.create(message='Invalid CIDs or no response from PubChem!'
                            )
                    sdf = None
            else:
        ...
        if not sdf:
            return render_to_response('submitCompound.html',
                    dict(input_mode=input_mode, post_data=request.POST,
                    p=page, matches=matches),
                    context_instance=RequestContext(request))
        ...
        if line.startswith('$$$$'):
            try:
                ...
                if name and name != '':
                    ...
                    moldata[namekey] = 'random' \
                        + str(random.randint(10000000, 90000000))
                if compid and compid != '':
                    moldata[idkey] = compid
                else:
                    moldata[idkey] = 'random' \
                        + str(random.randint(10000000, 90000000))
            ...
            while True:
                try:
                    idsearch = \
                        Compound.objects.get(cid__iexact=moldata[idkey].strip(),
                            library=library)
                except:

                    # search will call an exception IF the compound doesn't already exist

                    insert_single_compound(
                        moldata,
                        sdffile,
                        library,
                        namekey,
    ...
    for cid in cids:
        compound = \
            WorkbenchCompounds(compound=Compound.objects.get(cid__iexact=cid,
                               library=library), username=username)
        compound.save()
