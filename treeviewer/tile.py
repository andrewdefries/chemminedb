...
...."""
    ...
    if not os.path.exists(pdffile):
        ...
        if subl and subh:
            r_params.update(dict(level=subh, leaf=subl,
                            new_ref=new_ref))
    ...
    if subl and subh:
        if dref:
            raise HTTPFound(request.path + '?ref=' + new_ref + '&dref='
                            + dref)
    ...
    if dref:
        if 'drefs' not in session:
            session['drefs'] = dict()
        if dref not in session['drefs']:
            drefname = 'imported:' + file(os.path.join(WORK_DIR, dref
                    + '.desc')).read().strip()
            session['drefs'][dref] = drefname
        drefname = session['drefs'][dref]
        maxcol = len(file(os.path.join(WORK_DIR, dref + '.userdata'
                     )).readline().split('\t'))
    else:
        drefname = None
        maxcol = 0
...
def make_tile(img, tx, ty):
    ...
    if endx > bx or endy > by:
        ...
        img3.paste(img2, (0, 0, min(endx, bx) - startx, min(endy, by)
                   - starty))
        img2 = img3
    storage = StringIO()
