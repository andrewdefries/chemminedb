...
leaf_pattern = \
    re.compile(r"([-0-9.]+) ([-0-9.]+) Tm \(([a-zA-Z0-9_:.]+)\) Tj")
point_pattern = \
    re.compile(r"/F1 1 Tf 1 Tr [-0-9.]+ [-0-9.]+ [-0-9.]+ [-0-9.]+ ([-0-9.]+) ([-0-9.]+) Tm \(l\) Tj 0 Tr"
               )
stream_rec_pattern = re.compile(r"0 0 ([-0-9.]+) ([-0-9.]+) re f")
line_pattern = \
    re.compile(r"([-0-9.]+) ([-0-9.]+) m ([-0-9.]+) ([-0-9.]+) l S")
...
...."""
    ...
    def plot(self, indent=0):
        if self.top:
            self.top.plot(indent + 1)
        print '-' * indent + str(self.coord)
        if self.bottom:
            self.bottom.plot(indent + 1)
...
...."""
    ...
    def populate_data(node):
        if node.is_leaf():
            return dict(n_leaves=1, repr_leaf=node.data['label'],
                        height=0)
        else:
            t = 0
            ...
            node.data = dict(n_leaves=t, repr_leaf=topdata['repr_leaf'
                             ], height=topdata['height'] + 1)
            return node.data
...
...."""
    ...
    def ser_subtree(node):
        if node.is_leaf():
            all.append((node.coord.x, node.coord.y, None, None,
                       node.data))
            return len(all) - 1
        top = ser_subtree(node.top)
        bottom = ser_subtree(node.bottom)
        all.append((node.coord.x, node.coord.y, top, bottom, node.data))
        return len(all) - 1
