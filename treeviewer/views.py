...
def viewer(request, reference):
    try:
        context = index(request.session, reference,
                        mode=request.GET.get('mode', ''),
                        dref=request.GET.get('dref'))
        return render_to_response('treeviewer.html', context,
                                  context_instance=RequestContext(request))
    except SupportError:
        return render_to_response('treeviewer.html',
                                  dict(error='Sorry. The system encountered a problem while rendering your tree.'
                                  ),
                                  context_instance=RequestContext(request))
...
def userdata(request, reference):
    ...
    return HttpResponseRedirect(reverse('tree-viewer',
                                args=[reference]) + '?dref=' + dref
                                + '&mode=' + request.GET.get('mode', ''
                                ))
