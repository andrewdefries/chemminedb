...
def get_library_by_name(library, flush=False, cache_life=3600*24):
	...
	if lib is None:
		...
		lib = Library.objects.filter(
			header__name__iexact=library).select_related('header').annotate(
				count=Count('compound')).latest()
...
def _id_to_compound(lib, cid):
	...
	compound = Compound.objects.get(cid__iexact=cid.strip(), library=lib)
	return compound
...
def ajax(request):
	...
	if not func:
		...
		return HttpResponse(dumps(all_library_names()), mimetype="text/json")
	elif func == 'id_check':
		...
		try:
			...
			lib = compound.library.select_related('header').latest().header.name
...
def fuzzy_get_library_by_name(library):
	...
	# use django aggregation instead
	lib = Library.objects.filter(
		header__name__icontains=library).select_related('header').annotate(
			count=Count('compound')).latest()
...
def compound_screen(request, compound):
	...
	screen_stat = []
	accessible_screens = dict([(s.id, s) for s in get_all_screens(request)])
	screens = Screen.objects.extra(
...
@cache_page(60 * 5)
@vary_on_cookie		# prevent private compound to be seen
def compound_detail(request, library, cid, resource):
	...
	if resource and resource == 'property':
			p = '\n'.join(['%-50s %s' % (_[0][:50], _[1]) for _ in joelib_properties])
	...
	if 'addWorkbench' in request.POST:
		input_mode = 'view'
		addToWorkbench(compound=compound, username=request.user.username)
		matches = None
		request.user.message_set.create(message=
			"Compound added to workbench"
			)
...
def render_image(library, cid):
	...
	"""render the compound structure in the standard renderer"""
...
def annotation_search(request):
	...
	return render_to_response('search.html', dict(
		...
		fields=[(i.name.replace(' ', '__'), i.description)
			for i in all_property_fields()],
		),
		context_instance=RequestContext(request))
	...
	if request.method == 'GET':
		...
		if ajax:
			try:
				...
				return HttpResponse('{"status":"error"}', mimetype='text/plain')
			return HttpResponse('{"status":"ok"}', mimetype='text/plain')
		elif hash:
			try:
				compounds, info = load_async_struct_search_result(hash)
			except AsyncStructSearchNotReady:
			...
			info['fields']=[(i.name.replace(' ', '__'), i.description)
				for i in all_property_fields()]
			...
			return render_to_response(
				...
				dict(libraries=libraries, post_data=dict(smiles=smiles)),
				context_instance=RequestContext(request))
	else:
		input_mode = ''
		...
		if not libs:
			request.user.message_set.create(message=
				"No library selected. You must select at least one library."
			)
		...
		if not sdf:
			return render_to_response('structure_search.html', dict(
