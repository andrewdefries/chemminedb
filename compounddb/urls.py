...
all_libraries = LibraryHeader.objects.extra(select={'count':
	'select count(*) from compounddb_compound_library where library_id=(select compounddb_library.id from compounddb_library where compounddb_library.header_id = compounddb_libraryheader.id order by compounddb_library.created_time desc limit 1)'})
...
urlpatterns = patterns('',
	...
	url(r'^annotation/$',
		'compounddb.views.annotation_search',
		name="annotation-search"),
	url(r'^structure/$',
		'compounddb.views.structure_search',
		name="structure-search"),
	url(r'^(?P<library>[,a-zA-Z0-9 _-]+)/(:(?P<page>[0-9]+))?$',
		'compounddb.views.library_content', 
	...
	url(r'^$',
		object_list, dict(queryset=all_libraries), 
