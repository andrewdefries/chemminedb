...
try:
    ...
    sys.path.append('/home/lwang/library_upload/')
    import sdfiterator
...
def prepare_qsar_property_file(
    library_name,
    library_ver,
    qsar_raw_file,
    output_file,
    ...
    for line in fp:
        ...
        query = \
            'INSERT INTO compounddb_property (compound_id, value, field_id)'
        c1 = 0
        for value in values:
            try:
                name = index[c1]
...
def remove_property_in_file(lib_name, lib_ver, filename):
    ...
    for c in library.compound_set.all():
        query = \
            """delete from compounddb_property where compound_id=%s and field_id>42;""" \
            % c.id
        fp.write('%s\n' % query)
...
def _insert_property(compound, propertydata, tags):
    ...
    for key in propertydata.keys():
        if key in tags:
            try:
                j = \
                    Property(field=PropertyField.objects.get(source_tag=key),
                             value=propertydata[key], compound=compound)
                j.save()
            except:
...
def fast_insert_plate_test(
    ...
    for cmp in library.compound_set.all():
        try:
            ...
            raise
        p = Plate(format=format, plate=plate, well=well, compound=cmp)
        p.save()
        counter += 1
        print counter
...
def insert_fingerprint(
    ...
    for cmp in compounds:
        ...
        try:
            ...
            fp = Fingerprint(compound=cmp, md5=fpt_md5)
            fp.save()
            print '\r    %s' % count
        except:
            print cmp
