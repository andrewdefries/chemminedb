...
def add_properties(matches):
	...
	lookup = dict()
	for i in Property.objects.filter(compound__in=all_compounds
		).values_list('compound', 'field', 'value'):
...
def add_libname(base_queryset):
	return base_queryset.extra(
		select={'libname':
		'''select compounddb_libraryheader.name from
...
def add_molecular_weight(base_queryset):
	...
	return base_queryset.extra(
		select={'mw':'''select compounddb_property.value from
...
def do_annotation_search(base_queryset, query, field):
	...
	if field == 'cid':
		...
		base_queryset = base_queryset.extra(
			where=['compounddb_compound_info_index_col @@ to_tsquery(%s)'],
			params=["'" + query + "'"]).order_by()

	elif field == 'annotation':
		# use full text search in PostgreSQL
		base_queryset = base_queryset.extra(
			tables=['compounddb_annotation'],
			where=[
			  'compounddb_annotation.compound_id = compounddb_compound.id',
...
def do_property_search(base_queryset, field_name, operator, value, cache_life=3600):
	...
	if operator.strip() == '>':
		return base_queryset.filter(property__field=field, property__value__gt=value)
	if operator.strip() == '=':
		if not field.is_integer:
			raise SearchError(
				'Only integer property supports "=" query. Try ">" or "<"')
		return base_queryset.filter(property__field=field, property__value=value)
	if operator.strip() == '<':
		return base_queryset.filter(property__field=field, property__value__lt=value)
	else:
		raise SearchError(
		'Invalie operator: %s. Only "=", "<" or ">" are supported' % operator)
...
def search(query, page=1, request=None):
	from compounddb.views import get_library_by_name, fuzzy_get_library_by_name
	matches = []
	...
	if request:
	    base_queryset = base_queryset.filter(Q(username=request.user.username) | Q(username=''))
	else:
	...
	if m:
		lib = m.group(1).strip()
		try: library = get_library_by_name(lib)
		except:
			try:
				library = fuzzy_get_library_by_name(lib)
				if request:
					request.user.message_set.create(message=
					"%s does not match name of any library. %s is used instead"
	...
	# add plate/well
	if plate:
		base_queryset = do_annotation_search(base_queryset, plate, 'plate')
	if well:
...
def do_structure_search(base_queryset, sdf):
	return base_queryset.extra(
		tables=['compounddb_sdffile'],
		where=[
			'compounddb_sdffile.compound_id = compounddb_compound.id',
			'compounddb_sdffile.descriptor is not null',
			],
		select={'s':
		'''select sim(compounddb_sdffile.descriptor, (select ap(%s)))'''
		},
		select_params=[sdf],
		).order_by('-s')
...
class AsyncStructSearchNotReady(Exception): pass
class AsyncStructSearchDoesNotExist(Exception): pass
def async_struct_search(hash, sdf, library, search_term, cache_time=15*60):
